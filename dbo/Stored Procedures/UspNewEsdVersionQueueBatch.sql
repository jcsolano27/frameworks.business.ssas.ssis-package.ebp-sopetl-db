...
CREATE PROC dbo.[UspNewEsdVersionQueueBatch]
		@PlanningMonthId INT
...
*********************************************************************************/
	...
	/*----------------------	TEST HARNESS   --------------------------------------
		EXEC [esd].[UspNewEsdVersionQueueBatch]  
			@ReconMonthId = 216
	...
	DECLARE @DefaultCreationLoadGroups varchar(200)
		SELECT  @DefaultCreationLoadGroups = STUFF(
								(
									...
									WHERE GroupType = 'ESD'
										AND TableLoadGroupName <> 'Bonusback'
									For XML PATH('')
								),1,1,''
							)	
		...
		IF NOT EXISTS (
							...
							AND		T2.PlanningMonthId = @PlanningMonthId
						)
			BEGIN
				EXECUTE dbo.[UspEsdCreateEsdVersion]
