...
----    Purpose:        This proc is used to load data from Compass to [dbo].[CompassEohWithoutExcess] table   
----                        Source:  [dbo].[StgCompassMeasure]     
----                        Destination: [dbo].[CompassEohWithoutExcess]
...
CREATE   PROC [dbo].[UspEtlLoadCompassEohWithoutExcess]
    @Debug TINYINT = 0
...
BEGIN TRY
	...
	EXEC dbo.UspEtlMergeTableLoadStatus
      ...
      , @TableName = 'dbo.CompassEohWithoutExcess'
	  , @ProcessingStarted = 1
      , @BatchId = @BatchId
    ...
    DELETE FROM [dbo].[CompassEohWithoutExcess] WHERE EsdVersionId = @EsdVersionId;

    INSERT INTO [dbo].[CompassEohWithoutExcess](
		EsdVersionId,
	...
	(
		...
		(
			...
			WHERE M.MeasureNm = 'v_fg_eoh_wo_excess_units'
	...
	SELECT @RowCount = COUNT(*) FROM [dbo].[CompassEohWithoutExcess] WHERE EsdVersionId = @EsdVersionId;
