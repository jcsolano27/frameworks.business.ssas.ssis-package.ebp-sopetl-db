CREATE PROC [dbo].[UspEsdQueueBatch]
	 @EsdVersionID INT 
	,@TableLoadGroupsList VARCHAR(1000) 
	,@CompassPublishLogId INT = NULL
	,@Debug TINYINT = 0
...
/*********************************************************************************
	...
	Updated by:		Juan Solano

	Modified on:    8/01/2022

    Purpose:        Receive input from ESD Workbook - Manage ESD Versions UI and schedules ESD Batches to schedule
					ESD Data Extract batches via dbo.UspEtlQueueBatchRun procedure
...
***************************************************************************-
	...
	2020-12-04	Jeremy Webster		Added logic to return output parameter for each BatchRunId created for ESD Version - mapped in dbo.GuiUIEsdVersionBatchIds
	2020-12-16	Jeremy Webster		Added additional metadata creation logic to track status of data loads
	2021-01-06	Jeremy Webster		Changed name of gui.UIDataLoadEvent to dbo.GuiUIDataLoadRequest
...
EXEC [dbo].[UspEsdQueueBatch]
    @EsdVersionID = 31
  , @TableLoadGroupsList = '1|2|3|4|5'
  , @Debug = 1
SELECT * FROM Esd.EsdVersions
SELECT * FROM dbo.GuiUIEsdVersionBatchIds
...
BEGIN TRY
	...
	DECLARE @EsdBaseVersionID int
			...
			, @TableList VARCHAR(8000);
			    ...
			    -- used to get user name and not allow running stitch **************************************************
	...
	IF @UserRole = 'None' BEGIN
		SET @TableLoadGroupsList = REPLACE(@TableLoadGroupsList,'10','25')
		SET @TableLoadGroupsList = REPLACE(@TableLoadGroupsList,'11','25')
	END
    -- used to get user name and not allow running stitch **************************************************
	...
	SELECT @ReconMonthId = PlanningMonthId FROM dbo.EsdBaseVersions WHERE EsdBaseVersionId = @EsdBaseVersionId
	SELECT @ReconMonth = YearMonth FROM dbo.v_ESDCalendar WHERE MonthId = @ReconMonthId
	SELECT @PriorMonth = YearMonth FROM dbo.v_ESDCalendar WHERE MonthId = @ReconMonthId - 1
	SELECT @PriorMonthStartWw = MIN(YearWw) FROM dbo.v_ESDCalendar WHERE MonthId = @ReconMonthId - 1;
	...
	BEGIN 
		...
		IF(@LoadParams LIKE '%YearWw%')
			BEGIN
				SELECT @WorkingYearWw = @QtrstartWW --@PriorMonthStartWw --@QtrstartWW
			END
		ELSE
			BEGIN
		...
		IF (@Debug >= 1)
		BEGIN
			SELECT @SrcAppName as SrcAppName,@SrcVersionId as SrcVersionId, @EsdVerId as EsdVersionId, @EsdBaseVerId as ESDBaseVersionId, @TableLoadGroupId as TableLoadGroupId,@Ordinal AS Ordinal, @WorkingYearMm as YearMm, @WorkingYearWw as YearWw, @CompassPublishLogId as CompassPublishLogId,@TableList AS TableList
