...
-- =============================================
CREATE PROCEDURE dbo.UspProfitCenterDistribution
	@InputTable AS udtt_PcDistributionIn READONLY,
...
/*	TEST HARNESS
	...
	EXEC dbo.UspProfitCenterDistribution @InputTable, @DemandForecastMonth
*/
...
--*/
		...
		DECLARE @OutputTable AS udtt_PcDistributionOut 
		...
		INSERT #DemandForecast
			SELECT	distinct df.SnOPDemandProductId, df.ProfitCenterCd, ic.YearWw, df.Quantity / m.WeekCnt AS Quantity, ic.WwId
...
--into #tmp			
		...
		INSERT	@OutputTable
			SELECT	p.SnOPDemandProductId, p.YearWw, p.ProfitCenterCd, i.Quantity * p.[Percent] AS Quantity
			FROM	@InputTable i
					INNER JOIN #AllPcPercent p ON p.SnOPDemandProductId = i.SnOPDemandProductId AND p.YearWw = i.YearWw 

		INSERT	@OutputTable
			SELECT	SnOPDemandProductId, YearWw, 0, Quantity
			FROM	@InputTable i
			WHERE	NOT EXISTS (SELECT * FROM @OutputTable WHERE SnOPDemandProductId = i.SnOPDemandProductId AND YearWw = i.YearWw)
