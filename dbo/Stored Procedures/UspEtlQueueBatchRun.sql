...
CREATE PROC [dbo].[UspEtlQueueBatchRun]
    @Debug TINYINT = 0
...
--TRUNCATE TABLE dbo.EtlBatchRuns
...
SELECT TOP 1 * FROM dbo.EtlBatchRuns ORDER BY BatchRunId DESC;
...
BEGIN TRY
	...
	SELECT @LoadParameters = 
		...
		+ CASE WHEN @Datetime IS NOT NULL THEN '|Datetime' ELSE '' END
		+ CASE WHEN @CompassPublishLogId IS NOT NULL THEN '|CompassPublishLogId' ELSE '' END
		;
	...
	SELECT @ParameterList = 
		...
		+ ISNULL('|CompassPublishLogId=' + CAST(@CompassPublishLogId AS VARCHAR(1000)), '')
		;
	...
	IF(ISNULL(LEN(@LoadParameters),0) = 0)
		RAISERROR('Could not build LoadPxarameters based on parameters passed in',16,1);
	...
	--IF EXISTS (SELECT 1 FROM dbo.EtlBatchRuns bc WHERE bc.SourceApplicationId = @SourceApplicationId AND bc.ParameterList = @ParameterList AND bc.BatchRunStatusId IN (1,2,3) AND bc.TestFlag = @TestFlag)
	--	RAISERROR('Passed in @SourceApplicationId: %d @SourceVersionId: %s already exists in a ready or processing state.  Review dbo.EtlBatchRun.',16,1, @SourceApplicationId, @ParameterList);
	...
	BEGIN
		...
		BEGIN
			SELECT 
				...
				, ErrorReason = 
					CASE 
						WHEN t.TableName IS NULL THEN 'Table does not exist in dbo.EtlTables for given SourceApplicationId' 
						...
						WHEN t.Active = 0 AND @TestFlag = 0 THEN 'Can not queue disabled dbo.EtlTables with TestFlag=0.  Review Active flag in dbo.EtlTables'
					ELSE 'UNKNOWN' END
			FROM STRING_SPLIT(@TableList,'|') ss
			LEFT JOIN dbo.EtlTables t
				ON t.TableName = ss.value
		...
		RAISERROR('Invalid table passed in.  Please insure all tables exist in dbo.EtlTables and have the correct @LoadParameters specified.  Requeue with @Debug=1 for more details.',16,1);
	END
