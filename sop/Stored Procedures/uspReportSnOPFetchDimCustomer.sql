...
CREATE PROCEDURE [sop].[uspReportSnOPFetchDimCustomer]
(
    @Debug BIT = 0,
...
/********************************************************************************
    ...
    Purpose: this proc is used to get customer dimension data for SnOP Planning Forum Dashboards
    Main Tables:
...
BEGIN
    /*  TEST HARNESS
        EXECUTE [sop].[uspReportSnOPFetchDimCustomer] 1--, 202305, 202306
    */
 ...
 ;WITH CustomerIdList AS
    ...
    SELECT 
        ...
        ,c.CustomerTypeCd
        ,ss.SourceSystemNm
    FROM CustomerIdList cl
        INNER JOIN sop.Customer c
...
IF EXISTS (SELECT 1 FROM sysusers WHERE name = 'AMR\ebp sdra datamart svd tool pre-prod')
  BEGIN
    GRANT EXECUTE ON [sop].[uspReportSnOPFetchDimCustomer] to [AMR\ebp sdra datamart svd tool pre-prod]
...
IF EXISTS (SELECT 1 FROM sysusers where name = 'AMR\ebp sdra datamart svd tool prod')   
  BEGIN
    GRANT EXECUTE ON [sop].[uspReportSnOPFetchDimCustomer] to [AMR\ebp sdra datamart svd tool prod]
    PRINT 'Granted EXEC to [AMR\ebp sdra datamart svd tool prod]'
  END
IF EXISTS (SELECT 1 FROM sysusers WHERE name = 'GER\sys_dst')
  BEGIN
    GRANT EXECUTE ON [sop].[uspReportSnOPFetchDimCustomer] to [GER\sys_dst]
